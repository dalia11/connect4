...
public class Connect4Panel extends JPanel implements MouseListener,MouseMotionListener{
	...
	public Connect4Panel() throws InterruptedException{
	...
	public void paint(Graphics g){
		...
		if(con.getWin() == true){
			...
			g.setFont(new Font("Arial",Font.BOLD,18));
			g.setColor(Color.blue);
			g.drawString("         Blue To Move ", 50, 130);
			g.setFont(new Font("Arial",Font.BOLD,26));
			g.drawString("    Blue To Move ", 46, 470);
		}else if(player==2){
			g.setFont(new Font("Arial",Font.BOLD,18));
			g.setColor(Color.white);
			g.drawString("         White To Move ", 50, 130);
			g.setFont(new Font("Arial",Font.BOLD,26));
			g.drawString("    White To Move ", 46, 470);
		}
		}
		g.dispose();
}
